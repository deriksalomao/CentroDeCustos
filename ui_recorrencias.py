...
class RecorrenciasManager:
    ...
    def abrir_janela_recorrencias(self):
        ...
        def popular_tree():
            ...
            for idx, rec in enumerate(self.data_manager.recorrencias):
                tree.insert('', 'end', iid=idx, values=(
                    rec.get('descricao', ''), f"R$ {rec.get('valor', 0):.2f}", rec.get('tipo', ''),
                    rec.get('empresa', ''), rec.get('categoria', ''), rec.get('dia_mes', ''),
        ...
        def editar():
            ...
            self._popup_gerir_recorrencia(self.data_manager.recorrencias[int(selecionado)], int(selecionado), callback=popular_tree)

        def excluir():
            selecionado = tree.focus()
            if not selecionado: return
            if messagebox.askyesno("Confirmar", "Tem a certeza que deseja excluir esta recorrência?"):
                del self.data_manager.recorrencias[int(selecionado)]
                popular_tree()
                self.app.set_status("Recorrência excluída.")
    ...
    def _popup_gerir_recorrencia(self, dados=None, idx=None, callback=None):
        win = tk.Toplevel(self.app.root); win.title("Adicionar/Editar Recorrência")
        win.geometry("450x350"); win.transient(self.app.root); win.grab_set()
        ...
        campos['empresa'] = ttk.Combobox(frame, values=list(self.data_manager.dados_empresas.keys()), state='readonly'); campos['empresa'].grid(row=3, column=1, sticky='ew', pady=5)
        ttk.Label(frame, text="Categoria:").grid(row=4, column=0, sticky='w', pady=5)
        campos['categoria'] = ttk.Combobox(frame, values=self.data_manager.categorias, state='readonly'); campos['categoria'].grid(row=4, column=1, sticky='ew', pady=5)
        ...
        def salvar():
            try:
                rec = {'descricao': campos['descricao'].get(), 'valor': float(campos['valor'].get()),
                       'tipo': campos['tipo'].get(), 'empresa': campos['empresa'].get(),
                       'categoria': campos['categoria'].get(), 'dia_mes': int(campos['dia_mes'].get()),
                       'ultimo_lancamento': dados.get('ultimo_lancamento', 'Nunca') if dados else 'Nunca'}
                if not all([rec['descricao'], rec['tipo'], rec['empresa'], rec['categoria']]): raise ValueError("Preencha todos os campos.")
    ...
    def verificar_e_lancar_recorrencias(self):
        ...
        for rec in self.data_manager.recorrencias:
            ...
            if rec['ultimo_lancamento'] != 'Nunca':
                try: ultimo_lancamento = datetime.strptime(rec['ultimo_lancamento'], '%Y-%m-%d')
                except ValueError: continue
            
            lancar = (ultimo_lancamento is None) or ((ultimo_lancamento.year, ultimo_lancamento.month) != (hoje.year, hoje.month))
            if lancar and hoje.day >= rec['dia_mes']:
                data_lancamento = hoje.replace(day=rec['dia_mes'])
                novo_lancamento = pd.DataFrame([[data_lancamento, rec['empresa'], 'Geral', rec['categoria'], rec['descricao'], rec['tipo'], rec['valor']]], columns=self.data_manager.colunas)
                self.data_manager.df_lancamentos = pd.concat([self.data_manager.df_lancamentos, novo_lancamento], ignore_index=True)
                rec['ultimo_lancamento'] = hoje.strftime('%Y-%m-%d')
                mudancas_feitas = True
        
        if mudancas_feitas:
            self.app.atualizar_relatorio()
            self.app.set_status("Lançamentos recorrentes foram criados/atualizados.")
